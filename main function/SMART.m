function [indices, objfunVal] = SMART(DOF,varargin)
...
if DOF >= 2
    ...
    errordlg('Choose DOF greater or equal to 2','Error')
    return
end
...
while (numel(differences) ~= DOF+1) || any((differences) ~= DOFvec)
    ...
    uniqueDiffs = arrayfun(@(i)fliplr(setdiff(newDiffs(i,:), differences)), (1:length(posCan)).', 'UniformOutput', false);
